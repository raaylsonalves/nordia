...
export default function FlowField({
  ...
  useEffect(() => {
    ...
    // Read inside the effect, never during render — resolving this at render
    // time gives the server and the client different output and breaks hydration.
    const reduceMotion = window.matchMedia(
      "(prefers-reduced-motion: reduce)",
    ).matches;
    // Under reduced motion, advance the field on 1 frame in 6.
    const REDUCED_FRAME_SKIP = 6;
    ...
    const render = () => {
      // Added: soften for prefers-reduced-motion. Upstream runs this ~1200
      // particle loop at full speed regardless of the preference; freezing it
      // outright reads as a broken background. Skipping the update on most
      // frames keeps a slow drift — the field still breathes, much slower.
      // This has to sit at the TOP: the draw below has already happened by the
      // time the loop reaches the bottom, so gating there would slow nothing.
      frames += 1;
      if (reduceMotion && frames % REDUCED_FRAME_SKIP !== 0) {
      ...
      for (const p of particles) {
        ...
        // Upstream does `(p.hue + angle/(2π) * 70) % 360`, but `angle` is a sum
        // of sines reaching ~3π, so that term swings the hue by well over 100°
        // and walks straight out of the theme's band — "ember" renders green.
        // Wrap the drift inside [hueStart, hueStart + hueRange] instead.
        const drift = (angle / (Math.PI * 2)) * 18;
        const hueMod =
          cfg.hueStart +
          (((p.hue - cfg.hueStart + drift) % cfg.hueRange) + cfg.hueRange) %
            cfg.hueRange;
